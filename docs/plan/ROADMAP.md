...
| 1.2 | ✅ **Foundation loop builtin (commands)** — land 2026-08-17 | workflow เป็น change loop ของ claude-foundation ฝังใน binary ไม่ใช่ agents generic — built-in v1 plugin (pattern เดียวกับ 1.1) ฉีด `/investigate /change /build /prove /land /changes /feature /dev` เข้า `config.command` ตอน boot; เปิด default + opt-out `foundation_workflow: false`; user command ชื่อซ้ำชนะ; builtin resolve canonical instruction จาก PATH-selected `claude-foundation host instruction` protocol 1 และ fail closed โดยไม่ fallback ไป project files; **commands อย่างเดียว ไม่ฉีด agents** (per-phase model routing เลื่อนไปคู่ Router v2) | ~~M~~ done |
