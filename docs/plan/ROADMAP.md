...
| 1.2 | ✅ **Foundation loop builtin (commands)** — land 2026-08-17 | workflow เป็น change loop ของ claude-foundation ฝังใน binary ไม่ใช่ agents generic — built-in v1 plugin ฉีด 8 loop commands ตอน boot; เปิด default + opt-out `foundation_workflow: false`; user command ชื่อซ้ำชนะ; canonical instruction และ agent contract มาจาก checksum-pinned bundled Foundation โดย default พร้อม `changeloop foundation init/status/doctor/upgrade`; PATH เหลือเป็น explicit compatibility mode ผ่าน `foundation_runtime: "path"`; fail closed โดยไม่ fallback ไป project files | ~~M~~ done |
...
- **2026-08-17** ปิด version drift ของ 1.2: builtin templates เป็น marker และ
  resolve instruction ผ่าน `claude-foundation host instruction` protocol 1
  จาก CLI ใน `PATH`; boundary ผิดพลาดจะ fail closed โดยไม่ fallback ไป project
  command files
