...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            // .changeloop listed first so the reversal below applies it after
            // .opencode at the same level — changeloop wins, opencode keeps
            // working as a fallback.
            targets: [".changeloop", ".opencode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".opencode" || path.basename(item) === ".changeloop")
        .toReversed()
    ...
    // global config, project files, then `.changeloop`/`.opencode` files.
