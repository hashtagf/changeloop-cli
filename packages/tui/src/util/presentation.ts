// Pixel wordmark: left spells CHANGE, right spells LOOP with a
// circular-arrow loop icon standing in for both O letters.
const logo = {
  left: [
    " ████ █   █  ███  █   █  ████ █████",
    "█     █   █ █   █ ██  █ █     █    ",
...
export function sessionEpilogue(input: { title: string; sessionID?: string }) {
  ...
  return [
    ...
    `  ${weak("Continue")}${bold}changeloop -s ${input.sessionID}${reset}`,
    "",
  ].join("\n")
}
