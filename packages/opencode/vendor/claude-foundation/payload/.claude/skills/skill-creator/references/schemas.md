...
**Fields:** `grading_result` values: `"baseline"`, `"won"`, `"lost"`, or `"tie"`.

---

## grading.json

Output from the grader agent. Located at `<run-dir>/grading.json`. The `expectations` array **must** use `text`, `passed`, and `evidence` fields — the viewer depends on these exact names.
