...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run changeloop with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running changeloop server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CHANGELOOP_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CHANGELOOP_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
