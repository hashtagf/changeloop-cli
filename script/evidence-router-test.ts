#!/usr/bin/env bun
// Runs the scoped bun test suites for model-router-v1 (router config schema
// and router plugin tests) across packages/core and packages/opencode, then
...
// this reads the JUnit reports and re-emits counts.
...
const suites = [
  {
    ...
    files: ["config/router.test.ts"],
  },
  {
    cwd: path.join(root, "packages", "opencode"),
    files: ["plugin/router.test.ts"],
  },
]
...
for (const [index, suite] of suites.entries()) {
  const reportPath = path.join(resultsDir, `evidence-router-test-${index}.xml`)
...
await Bun.write(
  path.join(resultsDir, "evidence-router-test.json"),
  JSON.stringify({ tests: totalTests, failures: totalFailures }),
)

console.log(`evidence-router-test: ${totalTests} tests, ${totalFailures} failures`)
